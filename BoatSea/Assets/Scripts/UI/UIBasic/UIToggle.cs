...
public class UIToggle : MonoBehaviour {
    ...
    [SerializeField]
    string _checkedFrame = string.Empty;
    [SerializeField]
    string _uncheckedFrame = string.Empty;
    ...
    void Set(bool state) {
        if (!mStarted) {
            mChecked = state;
            startsChecked = state;
            SetFrameChecked(state);
        } else if (mChecked != state) {
            // Uncheck all other checkboxes
            if (radioButtonRoot != null && state) {
                UIToggle[] cbs = radioButtonRoot.GetComponentsInChildren<UIToggle>(true);
    ...
    void SetFrameChecked(bool isChecked) {
        if (checkSprite != null) {
            checkSprite.spriteName = isChecked ? _checkedFrame : _uncheckedFrame;
        }
    }
}